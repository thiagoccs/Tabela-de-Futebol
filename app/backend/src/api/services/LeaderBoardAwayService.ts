...
export default class LeaderBoardAwayService {
  ...
  public static totalDeVitorias(id: number, matches: IMatch[]): number {
    const vitorias = matches.filter((match) => {
      const { homeTeamGoals, awayTeamId, awayTeamGoals } = match;
      return (awayTeamId === id && awayTeamGoals > homeTeamGoals);
    }).length;
    return vitorias;
  }
  ...
  public static totalDePontos(id: number, matches: IMatch[]): number {
    const vitorias = LeaderBoardAwayService.totalDeVitorias(id, matches);
    const empates = LeaderBoardAwayService.totalDeEmpates(id, matches);
  ...
  async leaderBoardAway() {
    ...
    const result = LeaderBoardAwayService.calculoDePontos(times, partidas);
    const ordenado = LeaderBoardAwayService.ordenamento(result);
    return ordenado;
  }
}
